...
## Purpose
對父目錄下所有兄弟層 git repo 並行執行 `git pull`，以 `.env` 的 `SYNC_REPOS` 白名單指定同步清單；未設定時停下並引導執行 `sync-git init`，不掃描任何 repo。
## Requirements
### Requirement: 掃描兄弟層 git repo
工具 SHALL 以 `path.dirname(__dirname)`（sync-git 工具 repo 的父目錄）作為固定掃描根目錄，掃描該目錄下所有子資料夾找出含有 `.git` 目錄的 repo。工具 MUST NOT 使用 `process.cwd()` 或環境變數作為掃描根解析依據。`SYNC_REPOS_ROOT` 環境變數 SHALL NOT 再被讀取。
...
工具 SHALL 讀取 sync-git repo 根目錄內的 `.env` 或執行環境中的 `SYNC_REPOS`，若存在且包含 repo 名稱，則只對該清單內的 repo 執行 pull。`.env` 位置 MUST 以 `__dirname` 為錨點。`SYNC_REPOS_INCLUDE` SHALL NOT 再被讀取。當 `SYNC_REPOS` 未設定（無值或空值）時，工具 SHALL NOT 掃描全部 repo，而是停下並引導使用者執行 `sync-git init`。
